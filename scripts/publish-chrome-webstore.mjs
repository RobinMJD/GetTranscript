// Adapted from UseMyCurrentAccount++; see PUBLISHER_LICENSE.txt.
...
export function readChromeWebStoreConfig(env = process.env) {
  return {
    ...
    pollAttempts: readPositiveInteger(
      env.CHROME_WEBSTORE_UPLOAD_POLL_ATTEMPTS,
      20,
    ),
    pollIntervalMs: readPositiveInteger(
      env.CHROME_WEBSTORE_UPLOAD_POLL_INTERVAL_MS,
      15_000,
    ),
  };
}
...
export function isUploadInProgress(payload) {
  return [
    "IN_PROGRESS",
    "ITEM_UPLOAD_STATE_IN_PROGRESS",
    "UPLOAD_IN_PROGRESS",
  ].includes(getUploadState(payload).toUpperCase());
}

export function isUploadSuccess(payload) {
  return ["SUCCEEDED", "SUCCESS", "ITEM_UPLOAD_STATE_SUCCEEDED"].includes(
    getUploadState(payload).toUpperCase(),
  );
}

export function isUploadFailure(payload) {
  return ["FAILED", "FAILURE", "ITEM_UPLOAD_STATE_FAILED"].includes(
    getUploadState(payload).toUpperCase(),
  );
}
...
export function sanitizeChromeWebStoreMessage(value) {
  return String(value || "")
    .replace(
      /((?:refresh_token|access_token|client_secret)=)[^\s&]+/gi,
      "$1[redacted]",
    )
    .replace(
...
async function main() {
  ...
  if (missing.length)
    throw new Error(
      `Missing required Chrome Web Store configuration: ${missing.join(", ")}.`,
    );
  ...
  const currentStatus = await fetchJson(endpoints.fetchStatusUrl, {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  if (isPendingSubmission(currentStatus))
    throw new Error(
      "An existing Chrome submission is pending review. Resolve that review before submitting another package.",
    );
  ...
  console.log(
    `Chrome Web Store publish submitted for ${basename(config.zipPath)}.`,
  );
...
async function refreshAccessToken(tokenUrl, config) {
  ...
  if (!payload.access_token || typeof payload.access_token !== "string") {
    throw new Error(
      `Chrome Web Store token response did not include an access token: ${sanitizeChromeWebStoreMessage(JSON.stringify(payload))}`,
    );
  }
...
async function pollUploadStatus(
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    console.log(
      `Chrome Web Store upload is still processing (${attempt}/${attempts})...`,
    );
  }
  throw new Error(
    "Chrome Web Store upload did not finish processing before the polling timeout.",
  );
}
...
function assertUploadReady(payload) {
  ...
  if (isUploadFailure(payload)) {
    throw new Error(
      `Chrome Web Store upload failed: ${sanitizeChromeWebStoreMessage(JSON.stringify(payload))}`,
    );
  }
  const state = getUploadState(payload) || "unknown";
  throw new Error(
    `Chrome Web Store upload did not return a successful state (${state}): ${sanitizeChromeWebStoreMessage(JSON.stringify(payload))}`,
  );
}
...
async function fetchJson(url, init) {
  ...
  if (!response.ok) {
    throw new Error(
      `Chrome Web Store API request failed (${response.status}): ${sanitizeChromeWebStoreMessage(responseText)}`,
    );
  }
  return payload;
}
