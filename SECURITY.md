...
The manifest grants only `activeTab`, `scripting`, `downloads` and `storage`. No content scripts run continuously. No externally connectable messaging is exposed. The extension page CSP permits packaged scripts only and blocks remote connections. Page-local caption fetches are restricted to referenced HTTP(S) and blob track URLs and use normal browser CORS and authentication rules.
